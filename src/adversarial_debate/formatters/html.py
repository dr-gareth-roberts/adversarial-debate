...
class HTMLFormatter(Formatter):
    ...
    def _html_header(self) -> str:
        ...
        tool_name = escape(self.config.tool_name)
        tool_version = escape(self.config.tool_version)
        generated_at = datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S UTC")
        tool_meta = f"Generated by {tool_name} v{tool_version}"
...
<head>
    ...
    <title>Security Analysis Report - {tool_name}</title>
    <style>
        :root {{
            --bg-primary: #1a1a2e;
            --bg-secondary: #16213e;
...
"""
    ...
    def _html_verdict(self, verdict: dict[str, Any]) -> str:
        ...
        if summary.get("report"):
            report = escape(str(summary["report"]))
            verdict_html += f'<p style="text-align: left; margin-top: 1rem;">{report}</p>'
    ...
    def _html_finding(self, finding: dict[str, Any], index: int) -> str:
        ...
        if remediation:
            remediation_text = escape(str(remediation))
            html += (
                f'<div class="remediation"><strong>💡 Remediation:</strong> '
                f"{remediation_text}</div>"
            )
    ...
    def _html_footer(self) -> str:
        ...
        return f"""
        <footer>
            <p>Generated by <strong>{tool_name}</strong> v{tool_version}</p>
